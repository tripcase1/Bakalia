...
import { 
  ShieldAlert, Mail, Lock, Phone, ArrowRight, Loader2, Compass, AlertCircle
} from "lucide-react";
...
export default function LoginPage() {
  ...
  const handleSendOtp = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!formattedPhone.startsWith("+")) {
        formattedPhone = "+88" + formattedPhone; // Default to Bangladesh country code
      }
  ...
  return (
    <div className="flex-1 flex flex-col justify-center items-center px-4 py-16 bg-[#F4F6F9] dark:bg-[#010818] transition-colors duration-300">
      ...
      {/* Hidden container for Google Recaptcha */}
      ...
      <div className="w-full max-w-md bg-white dark:bg-[#01205B] border border-slate-200 dark:border-slate-800 rounded-2xl p-6 shadow-xl backdrop-blur-xl">
        <div className="text-center mb-6">
          ...
          <h2 className="text-lg font-black text-slate-905 dark:text-white">
            {language === "en" ? "Sign In to Bakalia" : "বাকলিয়া পোর্টালে লগইন করুন"}
          </h2>
          <p className="text-[10px] text-slate-400 dark:text-slate-500 uppercase font-black tracking-wider mt-1">
            {language === "en" ? "Smart Community Operating System" : "স্মার্ট নাগরিক প্রবেশদ্বার"}
          </p>
        </div>
        ...
        <div className="flex bg-slate-50 dark:bg-[#010818] p-1 rounded-lg border dark:border-slate-800/30 mb-5">
          ...
          >
            {language === "en" ? "Phone OTP" : "মোবাইল কোড"}
          </button>
        </div>
        ...
        {method === "email" ? (
          <form onSubmit={handleEmailLogin} className="space-y-4">
            ...
            >
              ...
              <span>{language === "en" ? "Sign In" : "লগইন করুন"}</span>
            </button>
          </form>
        ) : (
          /* Phone OTP SMS flow */
          <form onSubmit={otpSent ? handleVerifyOtp : handleSendOtp} className="space-y-4">
            ...
            >
              {loading ? <Loader2 className="w-4 h-4 animate-spin" /> : <ArrowRight className="w-4 h-4" />}
              <span>{otpSent ? (language === "en" ? "Verify Code" : "কোড নিশ্চিত করুন") : (language === "en" ? "Send SMS OTP" : "এসএমএস কোড পাঠান")}</span>
            </button>
          </form>
        )}
        ...
        <p className="text-[11px] text-center text-slate-400 dark:text-slate-500 mt-6 font-bold">
          {language === "en" ? "Don't have an account?" : "নতুন নাগরিক?"}{" "}
